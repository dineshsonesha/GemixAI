...
export default function WriteArticle() {
  ...
  const onSubmitHandler = async(e) => {
    ...
    try {
      setLoading(true);
      const prompt = `Write an article on ${input} with a length of around ${selectedLength.length} words. Make sure the article is unique and plagiarism free.`;
  ...
  return (
    <div className='h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700'>
      ...
      <form onSubmit={onSubmitHandler} className='w-full max-w-lg p-4 bg-white border border-gray-200 rounded-lg'>
        ...
        <input onChange={(e)=> setInput(e.target.value)}  value={input} type='text' placeholder='The future of artificial intelligence is...' className='w-full p-2 px-3 mt-2 outline-none text-sm border border-gray-300 rounded-md' required />
