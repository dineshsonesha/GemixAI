...
export default function GenerateImages() {
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      setLoading(true);
      const prompt = `Generate an image with the description ${input} in the ${selectedStyle} style.`;
  ...
  return (
    <div className='h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700'>
      ...
      <form onSubmit={onSubmitHandler} className='w-full max-w-lg p-4 bg-white border border-gray-200 rounded-lg'>
        ...
        <textarea onChange={(e) => setInput(e.target.value)} value={input} rows={4} placeholder='Describe what you want to see in the image...' className='w-full p-2 px-3 mt-2 outline-none text-sm border border-gray-300 rounded-md' required />
